# -*- coding: utf-8 -*-
"""
Created on Mon Oct  7 16:14:45 2024
...
deltahe=0.1;%0.1 pour init=2 ; %0.1025 pour init=4
hr=0.2-deltahe;%0.2-deltahe pour init=2 ; 0.15-deltahe pour init=4
...
for i=1:SY
    if dx==0.008
        bathy(i,SX-floor(3/4*250)+1:SX)=hr-m*[1:floor(3/4*250)];%res8
    elseif dx==0.006
        bathy(i,SX-250+1:SX)=hr-m*[1:250];%res6
    end
...
%X1=510;
%Y1=380;
Y=[50 50 50 50 50 50 50 50 50];%res0.5
% XX=[1015 1018 1021 1024];%res2
% XX=[SX-176 SX-163 SX-151 SX-126 SX-101 SX-76 SX-51 SX-26];%res2
XX=[SX-176 SX-163 SX-151 SX-126 SX-101 676 826 451 551];
...
%             for k=2:SXb-1
%                 test(j,k)=0;
%                 if ind(j,k)==1 ;i=i+1;end;end;end
...
save etatout_25_gages_L_9m_init_4 etatout
save uout_25_gages_L_9m_init_4 uout
save vout_25_gages_L_9m_init_4 vout
save gages_25_gages_L_9m_init_4 c
...
close all;
for i=1:ss
    figure;hold on;imagesc(xb(650:850),yb,vort(:,650:850,i))
%     caxis([-0.001 0.001])µ%etatout
    caxis([-30 30])
    %vort : caxis([-30 30])
    %u caxis([-0.6 0.6])
    %v caxis([-0.3 0.3])
    %etat caxis([-0.06 0.06])
    axis equal     
    xlabel('x (in m)')                                          %On place les titres des axes                        
    ylabel('y (in m)')
    zlabel('z (in m)')
...
writerObj = VideoWriter('SV_mangrove_16cyl_Linit_2m_vort.avi');
